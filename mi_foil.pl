...
/*                Learning, 1991                                  */
/*                                                                */
/*  call        : foil(Name/Arity), foil_time(Name/Arity),        */
/*                foil(Predicate), foil_time(Predicate)           */
/*                                                                */
...
% Modificaciones de Miguel A. Gutiérrez Naranjo
% 2 de Marzo de 2008

% Modificaciones en best_next_clause
% Modificacaciones en get_example
% Modificado compute_gain

% Modificaciones de Miguel A. Gutiérrez Naranjo
% 2 de Marzo de 2008
% Modificaciones en info_value
% Nuevo predicado informa/6
% Nuevo parámetro: verbosity
% - verbosity(0) Devuelve sólo la ganancia de información
% - verbosity(1) Devuelve el número de ejemplos positivos y negativos
%   cubiertos
% - verbosity(2) Devuelve también la lista de ejemplos positivos y
%   negativos cubiertos.
% Modificaciones en foil_loop
% Modificaciones en foil/1
% Modificaciones en extend_clause_loop
% Modificaciones en compute_gain

/******************************************************************/
/*                                                                */
/*  call        : foil_time (+PREDICATE)                          */
/*                                                                */
...
/*                                                                */
/******************************************************************/
/* Run FOIL and print run time statistics.                        */
/******************************************************************/
foil_time(Name/Arity) :-
...
foil_time(Goal) :-
	...
	time(T1), 
	T is T1 - T0,
	write('Run Time = '), write(T), write(' sec.'), nl.
...
foil(Goal) :-
	foil(Goal,Clauses),
	nl, write('Definición encontrada:'), nl, 
...
get_examples(Goal, Pos, Neg) :-
	...
	( clause(foil_cwa(true),true) ->
	      create_negatives(Pos, Neg)
% He comentado la siguiente instruccion y la he sustituido por otra	
%	; findall(Goal, clause(not(Goal),true), Neg) 
	; findall(Goal, clause(Goal,fail), Neg) 
	).	      
...
foil_loop(Pos, Goal, Neg, Clauses0, Clauses) :-
	( Pos = [] ->
	      Clauses = Clauses0
	; nl, write('Ejemplos positivos no cubiertos:'), nl, 
	  write(Pos), nl, 
	  nl, write('Añadiendo una regla ...'), nl, nl,
	  extend_clause_loop(Neg, Pos, (Goal :- true), Clause),
	  nl, write('Regla encontrada:'), nl,
...
extend_clause_loop(Nxs0, Pxs0, Clause0, Clause) :-
	( Nxs0 = [] ->
	      Clause = Clause0
	; write('******************************************************'),nl,
	write('Especializando la regla: '),
	\+ \+ (numbervars(Clause0,0,_),print(Clause0)),
	  nl, write('- Negativos cubiertos:'), nl, write(Nxs0), nl,
	  nl, write('- Positivos cubiertos:'), nl, write(Pxs0), nl, nl,
	  generate_possible_extensions(Clause0, Ls),
	  info_value(Clause0, Pxs0, Nxs0, Info,_,_,_,_),
...
% Compute the information matrix for the set of positive and negative
% tuples which result from applying Clause to the examples Pxs and
% NXs. 


info_value(Clause, Pxs, Nxs, Value,Ptuples,P,Ntuples,N) :-
...
% Nuevo predicado informa/6
informa(0,_,_,_,_,_,_,_).
informa(1,T,_,Clause,_,P,_,N):-
	write('Regla: '),
	\+ \+ (numbervars(Clause,0,_),print(Clause)),
	nl,
	write('Número de ejemplos positivos: '), write(P),nl,
	write('Número de ejemplos negativos: '), write(N),nl,
	write('Número de pos. cubiertos en la cláusula ampliada: '),
	write(T),nl.
informa(2,T,Retained,Clause,Ptuples,P,Ntuples,N):-
	write('Regla: '),
	\+ \+ (numbervars(Clause,0,_),print(Clause)),
	nl,
	write('Número de ejemplos positivos: '), write(P),nl,
	write('Ejemplos positivos: '), write(Ptuples),nl,
       	write('Número de ejemplos negativos: '), write(N),nl,
       	write('Ejemplos negativos: '), write(Ntuples),nl,
	write('Número de pos. cubiertos en la cláusula ampliada: '),
	write(T),nl,
	write('Positivos cubiertos en la cláusula ampliada: '),
	write(Retained),nl.
...
%      He descomentado las siguientes 3 líneas
	\+ \+ ( numbervars(Best1,0,_), 
		write('Ganancia: '), write(Gain1), write('	Regla: '), 
		print(Best1), nl,nl ),
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	( Gain1 > Gain0 ->
...
compute_gain(Nxs, Pxs, Info, Clause, Gain) :-
	...
	length(Retained, R),
        info_value(Clause, Pxs, Nxs, Info1,Ptuples,P,Ntuples,N),
	( R =:= 0 -> Gain = 0.0
	; Gain is R * (Info - Info1)
	),
	verbosity(Verb),
	informa(Verb,R,Retained,Clause,Ptuples,P,Ntuples,N).
...
%compound(X) :- \+( atomic(X) ; var(X) ).

help :- write('Load an example data file: [filename]'), nl,
	write('Start Foil               : foil(Predicate/Arity)'), nl,
	write('Start Foil measure time  : foil_time(Predicate/Arity)'), nl.
